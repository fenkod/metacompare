...
def compare_deck_to_mtggoldfish_metagame(deck_dict, metagame_collection):
    ...
    for metadeck in metagame_collection:
        if metadeck["deck"]:
            deck = counter_cosine_similarity(Counter(deck_dict['deck']), Counter(metadeck['deck']))
            deck_noland = counter_cosine_similarity(Counter(deck_dict['deck_noland']), Counter(metadeck['deck_noland']))
